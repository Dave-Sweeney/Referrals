...
AS
  BEGIN
    ...
    magellan_closed_date = @magellan_closed_date,
    magellan_contact_status = @magellan_contact_status,
    magellan_closed_status = @magellan_closed_status,
    magellan_closed_notes = @magellan_closed_notes,
    magellan_closed_reason = @magellan_closed_reason
  ...
  ELSE
    BEGIN
      ...
      SET
        ...
        phone = @parent_phone,
        ok_to_text = @parent_ok_to_text,
        email = @parent_email,
        comment = @parent_comment
      WHERE id = (SELECT parent_id FROM [dbo].[REFERRALS] WHERE referral_id = @referral_id)
    END
  END